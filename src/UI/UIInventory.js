...
export default class UIInventory{
    constructor(
        ...
        this.UICoordinates=[
            [220,540],[220,570],[380,540],[380,570],[540,540],[540,570]
        ]
        ...
        this.UIInventoryAccessBG = this.scene.add.rectangle(400,245,290,440,this.myColor.military.number,0.8).setVisible(false);
        for (let n = 0; n < this.bagMax; n++) {
            this.allItems.push(["No item","No type","",""]);
            this.allItemsNoteDisplay.push("");

            // each item box has [0]=rect for BG, [1]= X, [2]=Display name
            this.allItemsBox.push([
                this.scene.add.rectangle(400,70*(n+1),250,55,this.myColor.military.number,1).setVisible(false).setInteractive(),
                this.scene.add.text(490,58+70*n,"✖",this.textStyleAccess).setVisible(false).setInteractive(),
                this.scene.add.text(290,58+70*n,"Empty",this.textStyleAccess).setVisible(false)
            ]);
    ...
    InsertItem({
        ...
        extra='extra',
            ...
            if(this.bagNow<this.bagMax){

                //Find first empty slot box, then insert the new item
                let index=0;
                while (index<this.bagMax) {
                    if(this.allItems[index][0]=="No item"){
                        this.allItems[index][0]=name;
                        this.allItems[index][1]=type;
                        this.allItems[index][2]=desc;
                        this.allItems[index][3]=descLong;

                        this.itemBoxColorChange(type, index);
                        this.allItemsNoteDisplay[index] = "- "+name+": "+desc;
                        this.allItemsBox[index][2].setText(name+": "+type);
    ...
    EmptyItem(tween,target,index){
        ...
        sceneEvents.emit("destroy-item-ui",this.allItems[index][0]);
    ...
    AccessInventory(){
        ...
        if(this.isAccessOpen){
            ...
            document.querySelector("#indexNotes-p").textContent = "Hover '✖' for 5 seconds to\nitem delete permanently\n\nClick 'Bag' to close";
            document.querySelector("#indexNotes-h1").setAttribute("style","opacity:1;");
            document.querySelector("#indexNotes-p").setAttribute("style","opacity:1;");
